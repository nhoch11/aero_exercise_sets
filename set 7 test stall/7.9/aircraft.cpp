...
aircraft::aircraft(string filename)
{
    ...
    m_stall_effects = m_input["simulation"]["stall_effects"];
    m_blend_rate = m_input["simulation"]["blend_rate"];
    m_alpha_b = m_input["simulation"]["alpha_b"];
...
void aircraft::aerodynamics_aircraft(double* y, double* ans)
{
    ...
    // if gusts = true
        // double ihatg[3];
        // array_copy(&y[0],ihatg,3);
        // double gust_stuff = freq*lamgda*sin(stuff)...
        // double random = get_random
        // u += gust_stuff*random*ihatg[0]
        // v += gust_stuff*random*ihatg[1]
        // w += gust_stuff*random*ihatg[2]
    double p  = y[3]; 
    double q  = y[4];
    double r  = y[5];
    double xf = y[6]; 
    ...
    if (m_stall_effects == true && alpha != 0.0) {
        // model a flat plate
        double check = static_cast<double>(sign(alpha));
        double CL_plate = 2.0 * static_cast<double>(sign(alpha)) * sin(alpha) * sin(alpha) * cos(alpha);
        double CD_plate = 2.0 * pow(sin(abs(alpha)), (3 / 2));
        double Cm_plate = -0.8 * sin(alpha);

        // calculate sigma for a sigmoid blending function 
        double sigma = (1 + std::exp(-m_blend_rate * (alpha - m_alpha_b)) + std::exp(m_blend_rate * (alpha + m_alpha_b))) / ((1 + exp(-m_blend_rate * (alpha - m_alpha_b))) * (1 + exp(m_blend_rate * (alpha + m_alpha_b))));

        // blend the plate model with our pre stall model
        CL = (1 - sigma) * CL + sigma * CL_plate;
        CD = (1 - sigma) * CD + sigma * CD_plate;
        Cm = (1 - sigma) * Cm + sigma * Cm_plate;
}

...
public: // private variables and function can only be accessed inside the class
    f_16(std::string filename); // class constructor
    ~f_16(){}; // class destructor (default: releases memory)
...
private:
    ...
    void aerodynamics_f_16(double* y, double* ans);
    double get_sphere_CD(double reynolds);

    void f_16_rk4_func(double t, double* y, double* ans);
    void f_16_rk4(double t0, double* y0, double dt, int size, double* ans);

...
void aircraft::init_from_trim(){
    ...
    }else if (m_trim_type == "vertical"){
        printf("\nTrimming Aircraft for Vertical Climb or descent\n");
        // requires +90 or -90 climb angle
        // requires 
        gamma = m_input["initial"]["trim"]["climb_angle[deg]"];
        gamma *= pi/180.0;
    ...
    do {
        ...
        // update theta
        if (solve_Elevation == true & m_trim_type == "sct"){
        ...
        if (m_trim_type == "vertical"){
            
            // set bank angle to -pi
            if (gamma > 0.0){
                phi = 0.0;
                theta = pi/2;
            }
    ...
    gamma = asin((m_initial_state[0]*sin(theta) - (m_initial_state[1]*sin(phi) + m_initial_state[2]*cos(phi))*cos(theta))/m_V);
    //gamma = asin((m_initial_state[0]*sin(theta) - (-m_initial_state[2])*cos(theta))/m_V);
...
void aircraft::run_sim()
{
    ...
    FILE* out_file = fopen("hoch_EC.txt", "w");
    fprintf(out_file, "  Time[s]              u[ft/s]            v[ft/s]                w[ft/s]              p[rad/s]             q[rad/s]             r[rad/s]             x[ft]                y[ft]                z[ft]                e0                   ex                   ey                   ez                  gamma[deg]\n");
    ...
    do {
        ...
        quat_to_euler(&y[9], &euler[0]);
        V = sqrt(y[0]*y[0] + y[1]*y[1] + y[2]*y[2] );
        phi = euler[0];
        theta = euler[1];
        gamma = asin((y[0]*sin(theta) - (y[1]*sin(phi) + y[2]*cos(phi))*cos(theta))/V)*180.0/pi;
        fprintf(out_file, "%20.12e %20.12e %20.12e %20.12e %20.12e %20.12e %20.12e %20.12e %20.12e %20.12e %20.12e %20.12e %20.12e %20.12e %20.12e\n", t0, y0[0], y0[1], y0[2],y0[3],y0[4], y0[5], y0[6], y0[7], y0[8], y0[9], y0[10], y0[11], y0[12], gamma);
